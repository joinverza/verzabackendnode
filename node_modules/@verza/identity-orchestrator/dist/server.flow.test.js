...
void test("identity-orchestrator flow: create -> media -> run sync completes", async () => {
    ...
    const mediaByVerification = new Map();
    ...
    const pool = {
        query: async (sql, params = []) => {
            ...
            if (q.startsWith("insert into identity_verifications_v2")) {
                const [id, u, type, subjectId, status, createdAt, updatedAt] = params;
            ...
            if (q.startsWith("update identity_verifications_v2 set status='running'")) {
                ...
                if (!row)
                    return { rowCount: 0, rows: [] };
                if (!["created", "collecting_media", "failed"].includes(row.status))
                    return { rowCount: 0, rows: [] };
                row.status = "running";
                row.updated_at = updatedAt;
            ...
            if (q.startsWith("insert into identity_idempotency")) {
                const [_id, verificationId, key] = params;
            ...
            if (q.startsWith("select * from identity_idempotency")) {
                const [verificationId, key] = params;
    ...
    const { id: verificationId } = (await createVerification.json());
...
//# sourceMappingURL=server.flow.test.js.map