...
export function createConsentsRouter(ctx: MainApiContext): Router {
  ...
  router.get("/", async (req, res, next) => {
    try {
      const result = await ctx.pool.query(
        "select id,user_id,institution_id,credential_id,allowed_fields_json,purpose,status,expires_at,revoked_at,created_at from consents where user_id=$1 order by created_at desc",
        [req.auth.userId]
      );
  ...
  router.post("/", async (req, res, next) => {
    try {
      ...
      const inst = await ctx.pool.query<{ ok: number }>("select 1 as ok from institutions where id=$1 and status='active' limit 1", [
        body.institution_id
      ]);
      ...
      await ctx.pool.query(
        "insert into consents (id,user_id,institution_id,credential_id,allowed_fields_json,purpose,status,expires_at,created_at) values ($1,$2,$3,$4,$5,$6,$7,$8,$9)",
        [
          id,
          req.auth.userId,
          body.institution_id,
  ...
  router.get("/:consentId", async (req, res, next) => {
    try {
      ...
      const result = await ctx.pool.query(
        "select id,user_id,institution_id,credential_id,allowed_fields_json,purpose,status,expires_at,revoked_at,created_at from consents where id=$1 and user_id=$2 limit 1",
        [consentId, req.auth.userId]
      );
  ...
  router.post("/:consentId/revoke", async (req, res, next) => {
    try {
      ...
      const updated = await ctx.pool.query(
        "update consents set revoked_at=$1, status='revoked' where id=$2 and user_id=$3 and revoked_at is null",
        [now, consentId, req.auth.userId]
      );
