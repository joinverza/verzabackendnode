...
export function createProofsRouter(ctx) {
...
function buildProofPayload(opts) {
    const credential_digest = sha256Hex(canonicalJson({
...
function verifyProofEnvelope(ctx, input) {
    if (input.proof === undefined)
        return false;
    const seed = Buffer.from(ctx.config.RECEIPT_ED25519_SEED_B64, "base64");
    if (seed.length !== 32)
        throw badRequest("invalid_signing_seed", "Invalid signing seed");
