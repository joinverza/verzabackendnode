...
export function createAuthRouter(ctx) {
    ...
    router.post("/signup", createRateLimiter({ windowMs: 60_000, limit: 10 }), async (req, res, next) => {
        try {
            ...
            const ts = now();
            await ctx.pool.query("insert into users (id, tenant_id, email, name, password_hash, created_at, updated_at) values ($1,$2,$3,$4,$5,$6,$7)", [userId, DEFAULT_TENANT_ID, body.email.toLowerCase(), body.name, passwordHash, ts, ts]);
            const refreshToken = generateRefreshToken();
            const refreshHash = sha256Hex(refreshToken);
            const sessionId = randomUuid();
            const expiresAt = new Date(Date.now() + ctx.config.REFRESH_TOKEN_TTL_SECONDS * 1000);
            await ctx.pool.query("insert into sessions (id, user_id, refresh_token_hash, created_at, expires_at, user_agent, ip) values ($1,$2,$3,$4,$5,$6,$7)", [sessionId, userId, refreshHash, ts, expiresAt, String(req.headers["user-agent"] ?? ""), req.ip]);
    ...
    router.post("/login", createRateLimiter({ windowMs: 60_000, limit: 20 }), async (req, res, next) => {
        try {
            ...
            const result = await ctx.pool.query("select id,tenant_id,email,password_hash,role,twofa_enabled,twofa_secret,backup_codes_sha from users where email=$1 and status='active' limit 1", [body.email.toLowerCase()]);
            ...
            const ts = now();
            const expiresAt = new Date(Date.now() + ctx.config.REFRESH_TOKEN_TTL_SECONDS * 1000);
            await ctx.pool.query("insert into sessions (id, user_id, refresh_token_hash, created_at, expires_at, user_agent, ip) values ($1,$2,$3,$4,$5,$6,$7)", [sessionId, user.id, refreshHash, ts, expiresAt, String(req.headers["user-agent"] ?? ""), req.ip]);
    ...
    router.post("/refresh", createRateLimiter({ windowMs: 60_000, limit: 60 }), async (req, res, next) => {
        try {
            ...
            const result = await ctx.pool.query("select s.id, s.user_id, s.expires_at, s.revoked_at, u.role, u.email, u.tenant_id from sessions s join users u on u.id = s.user_id where s.refresh_token_hash=$1 and u.status='active' limit 1", [refreshHash]);
            const row = result.rows[0];
            if (!row)
                throw unauthorized("invalid_refresh", "Invalid refresh token");
            if (row.revoked_at)
                throw unauthorized("revoked_session", "Session revoked");
            if (row.expires_at.getTime() <= Date.now())
                throw unauthorized("expired_session", "Session expired");
    ...
    router.post("/forgot-password", createRateLimiter({ windowMs: 60_000, limit: 5 }), async (req, res, next) => {
        try {
            ...
            const result = await ctx.pool.query(email
                ? "select id,email,phone from users where email=$1 limit 1"
                : "select id,email,phone from users where phone=$1 limit 1", [email ?? phone ?? ""]);
            const user = result.rows[0];
            if (user) {
                const token = generateRefreshToken();
                const tokenHash = sha256Hex(token);
                const expiresAt = new Date(Date.now() + 60 * 60 * 1000);
                await ctx.pool.query("insert into password_reset_tokens (token_hash, user_id, expires_at, created_at) values ($1,$2,$3,$4) on conflict (token_hash) do nothing", [tokenHash, user.id, expiresAt, now()]);
    ...
    router.post("/reset-password", createRateLimiter({ windowMs: 60_000, limit: 10 }), async (req, res, next) => {
        try {
            ...
            const result = await ctx.pool.query("select user_id, expires_at, used_at from password_reset_tokens where token_hash=$1 limit 1", [tokenHash]);
            const row = result.rows[0];
            if (!row)
                throw badRequest("invalid_token", "Invalid token");
            if (row.used_at)
                throw badRequest("used_token", "Token already used");
            if (row.expires_at.getTime() <= Date.now())
                throw badRequest("expired_token", "Token expired");
...
function buildPasswordResetUrl(opts) {
...
async function trySendPasswordResetEmail(opts) {
    ...
    const transport = nodemailer.createTransport({
        ...
        auth: opts.ctx.config.SMTP_USER && opts.ctx.config.SMTP_PASS
...
async function trySendPasswordResetSms(opts) {
