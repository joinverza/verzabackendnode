...
export function createMeRouter(ctx: MainApiContext): Router {
  ...
  router.get("/", async (req, res, next) => {
    try {
      ...
      const result = await ctx.pool.query(
        "select id,email,name,phone,bio,avatar_url,did,role,status,created_at,updated_at from users where id=$1 limit 1",
        [userId]
      );
      const u = result.rows[0];
      if (!u) throw unauthorized("missing_user", "User not found");
      res.json({
        id: u.id,
        email: u.email,
        name: u.name,
  ...
  router.patch("/", async (req, res, next) => {
    try {
      ...
      await ctx.pool.query(
        "update users set name=coalesce($1,name), phone=coalesce($2,phone), bio=coalesce($3,bio), avatar_url=coalesce($4,avatar_url), did=coalesce($5,did), updated_at=$6 where id=$7",
        [body.name ?? null, body.phone ?? null, body.bio ?? null, body.avatar_url ?? null, body.did ?? null, ts, userId]
      );
      res.json({ status: "ok" });
    } catch (err) {
      next(err);
  ...
  router.post("/change-password", async (req, res, next) => {
    try {
      ...
      const result = await ctx.pool.query<{ password_hash: string }>("select password_hash from users where id=$1", [userId]);
      const row = result.rows[0];
      if (!row) throw unauthorized("missing_user", "User not found");
      const ok = await bcrypt.compare(body.current_password, row.password_hash);
      if (!ok) throw unauthorized("invalid_password", "Invalid password");
  ...
  router.get("/sessions", async (req, res, next) => {
    try {
      const userId = req.auth.userId;
      const result = await ctx.pool.query(
        "select id,user_id,created_at,expires_at,revoked_at,user_agent,ip from sessions where user_id=$1 order by created_at desc",
        [userId]
      );
  ...
  router.post("/2fa/verify", async (req, res, next) => {
    try {
      ...
      const userId = req.auth.userId;
      const result = await ctx.pool.query<{ twofa_secret: string }>("select twofa_secret from users where id=$1", [userId]);
      const row = result.rows[0];
      if (!row || !row.twofa_secret) throw badRequest("twofa_not_setup", "2FA not set up");
      const { verifyTotp } = await import("@verza/auth");
      const ok = verifyTotp({ secretBase32: row.twofa_secret, code: body.twofa_code });
      if (!ok) throw badRequest("invalid_twofa_code", "Invalid 2FA code");
  ...
  router.post("/2fa/disable", async (req, res, next) => {
    try {
      ...
      const userId = req.auth.userId;
      const result = await ctx.pool.query<{ password_hash: string; twofa_secret: string; backup_codes_sha: string }>(
        "select password_hash,twofa_secret,backup_codes_sha from users where id=$1",
        [userId]
      );
      const row = result.rows[0];
      if (!row) throw unauthorized("missing_user", "User not found");
      const okPass = await bcrypt.compare(body.password, row.password_hash);
      if (!okPass) throw unauthorized("invalid_password", "Invalid password");
