...
void test("fiat/stripe: webhook handles paid/refunded/disputed and idempotency", async () => {
  const sessionsById = new Map<string, PaymentSession>();
  const sessionsByPi = new Map<string, PaymentSession>();
  const webhookEventIds = new Set<string>();
  const ledgerEntries: Array<{ payment_id: string; amount_minor: number }> = [];

  const pool = {
    query: async (sql: string, params: unknown[] = []) => {
      const q = sql.replace(/\s+/g, " ").trim().toLowerCase();

      if (q === "begin" || q === "commit" || q === "rollback") return { rowCount: 1, rows: [] };

      if (q.startsWith("insert into stripe_webhook_events")) {
        const [eventId] = params as [string, string, string, Date];
        if (webhookEventIds.has(eventId)) return { rowCount: 0, rows: [] };
      ...
      if (q.startsWith("update fiat_payment_sessions set stripe_status=$1")) {
        const [stripeStatus, status, , id] = params as [string, string, Date, string];
        const sess = sessionsById.get(id);
        if (!sess) return { rowCount: 0, rows: [] };
      ...
      if (q.startsWith("insert into ledger_entries")) {
        const [, paymentId, amountMinor] = params as [string, string, number, string, string, Date];
        ...
        if (amountMinor < 0) {
          if (alreadyRefunded) return { rowCount: 0, rows: [] };
          ledgerEntries.push({ payment_id: paymentId, amount_minor: amountMinor });
          return { rowCount: 1, rows: [] };
        }
        if (alreadyPaid) return { rowCount: 0, rows: [] };
        ledgerEntries.push({ payment_id: paymentId, amount_minor: amountMinor });
        return { rowCount: 1, rows: [] };
      }
  ...
  app.use(
    "/api/v1/fiat/payments",
    createFiatPaymentsRouter({
      config: { STRIPE_WEBHOOK_SECRET: webhookSecret, RECEIPT_ED25519_SEED_B64: Buffer.alloc(32).toString("base64") } as any,
      logger,
      pool,
      stripe
    } as any)
  ...
  const postStripeWebhook = async (event: any) => {
    const payload = JSON.stringify(event);
    const stripeWebhooks = stripe.webhooks as unknown as {
      generateTestHeaderString: (opts: { payload: string; secret: string }) => string;
    };
...
void test("fiat/stripe: reconcile updates pending sessions and writes ledger", async () => {
  ...
  const fakeStripe = {
    paymentIntents: {
      retrieve: async (id: string) => {
        if (id === "pi_paid") return { id, status: "succeeded", charges: { data: [{ amount_refunded: 0 }] } };
        if (id === "pi_refunded") return { id, status: "succeeded", charges: { data: [{ amount_refunded: 100 }] } };
  ...
  app.use(
    "/api/v1/fiat/payments",
    createFiatPaymentsRouter({
      config: { JWT_SECRET: secret, JWT_ISSUER: "verza", JWT_AUDIENCE: "verza", RECEIPT_ED25519_SEED_B64: Buffer.alloc(32).toString("base64") } as any,
      logger,
      pool,
      stripe: fakeStripe
    } as any)
...
void test("admin/bridge: auth required and workflows behave", async () => {
  ...
  const logger = createLogger({ service: "main-api-test", level: "silent" });
  const app = createHttpApp({ logger, corsAllowedOrigins: [] });
  const ctx = { config: { JWT_SECRET: secret, JWT_ISSUER: "verza", JWT_AUDIENCE: "verza" }, logger, pool } as any;
  ...
  const invalidLimitBody = (await invalidLimit.json()) as any;
...
void test("admin/institutions: api keys and members workflows behave", async () => {
  ...
  const institutions = new Map<
    string,
    { id: string; tenant_id: string; name: string; status: string; created_at: Date; updated_at: Date }
  >();
  const apiKeys = new Map<string, { id: string; institution_id: string; key_hash: string; last4: string; revoked_at: Date | null }>();
  const users = new Map<string, { id: string; email: string; name: string }>();
  const members = new Map<string, { id: string; institution_id: string; user_id: string; role: string; status: string; created_at: Date }>();
  const memberByInstitutionUser = new Map<string, string>();
  ...
  const pool = {
    query: async (sql: string, params: unknown[] = []) => {
      ...
      if (q.startsWith("insert into institutions")) {
        const [id, tenantId, name, status, createdAt, updatedAt] = params as [string, string, string, string, Date, Date];
      ...
      if (q.startsWith("update institutions set status=$1")) {
        const [status, updatedAt, id, tenantId] = params as [string, Date, string, string];
        const row = institutions.get(id);
        if (!row || row.tenant_id !== tenantId) return { rowCount: 0, rows: [] };
        row.status = status;
        row.updated_at = updatedAt;
      ...
      if (q.startsWith("insert into institution_api_keys")) {
        const [id, institutionId, , keyHash, last4] = params as [string, string, string, string, string, Date];
      ...
      if (q.startsWith("select id, institution_id, name, last4, created_at, revoked_at from institution_api_keys")) {
        const [institutionId] = params as [string];
      ...
      if (q.startsWith("update institution_api_keys set revoked_at=$1")) {
        const [revokedAt, apiKeyId, institutionId] = params as [Date, string, string];
        const row = apiKeys.get(apiKeyId);
        if (!row || row.institution_id !== institutionId || row.revoked_at) return { rowCount: 0, rows: [] };
      ...
      if (q.startsWith("insert into institution_members") && q.includes("on conflict")) {
        const [id, institutionId, userIdParam, role, status, createdAt] = params as [string, string, string, string, string, Date];
      ...
      if (q.startsWith("update institution_members set role=coalesce")) {
        const [role, status, memberId, institutionId] = params as [string | null, string | null, string, string];
        const row = members.get(memberId);
        if (!row || row.institution_id !== institutionId) return { rowCount: 0, rows: [] };
  ...
  const apiKeyBody = (await apiKeyResp.json()) as { id: string; api_key: string; last4: string };
  ...
  const memberBody = (await memberCreate.json()) as { id: string };
  ...
  const memberRows = (await memberList.json()) as any[];
  ...
  const invalidUpdateBody = (await invalidUpdate.json()) as { error: { code: string } };
