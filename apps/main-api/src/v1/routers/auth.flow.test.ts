...
void test("auth flow: signup -> login -> refresh -> logout", async () => {
  ...
  const sessionsByRefreshHash = new Map<string, any>();
  ...
  const pool = {
    query: async (sql: string, params: unknown[] = []) => {
      ...
      if (q.startsWith("insert into users")) {
        const [id, tenant_id, email, name, password_hash] = params as [string, string, string, string, string];
        const row = {
          id,
          tenant_id,
          email,
      ...
      if (q.startsWith("insert into sessions")) {
        const [id, user_id, refresh_token_hash, created_at, expires_at, user_agent, ip] = params as [
          string,
          string,
          string,
          Date,
      ...
      if (q.startsWith("update sessions set refresh_token_hash=$1")) {
        const [newHash, newExpiresAt, id] = params as [string, Date, string];
      ...
      if (q.startsWith("update sessions set revoked_at=$1 where id=$2 and user_id=$3")) {
        const [revokedAt, id, userId] = params as [Date, string, string];
        const s = sessionsById.get(id);
        if (s && s.user_id === userId) s.revoked_at = revokedAt;
        return { rowCount: 1, rows: [] };
      }

      if (q.startsWith("update sessions set revoked_at=$1 where user_id=$2")) {
        const [revokedAt, userId] = params as [Date, string];
