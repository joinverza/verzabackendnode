...
export function createAdminInstitutionsRouter(ctx) {
    ...
    router.post("/:institutionId/api-keys", requirePermission(ctx, { action: PERMISSIONS.ADMIN_INSTITUTIONS_API_KEYS_CREATE }), async (req, res, next) => {
        try {
            ...
            await ctx.pool.query("insert into institution_api_keys (id, institution_id, name, key_hash, last4, created_at) values ($1,$2,$3,$4,$5,$6)", [id, institutionId, body.name ?? "admin_created", keyHash, last4, new Date()]);
            await appendAuditEvent(ctx.pool, {
                tenantId: req.auth.tenantId,
    ...
    router.post("/:institutionId/api-keys/:apiKeyId/revoke", requirePermission(ctx, { action: PERMISSIONS.ADMIN_INSTITUTIONS_API_KEYS_REVOKE }), async (req, res, next) => {
        try {
            const { institutionId, apiKeyId } = apiKeyIdParamsSchema.parse(req.params);
            const now = new Date();
            const updated = await ctx.pool.query("update institution_api_keys set revoked_at=$1 where id=$2 and institution_id=$3 and revoked_at is null", [now, apiKeyId, institutionId]);
            if (!updated.rowCount)
...
//# sourceMappingURL=adminInstitutions.js.map