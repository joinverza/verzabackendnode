...
export async function createIdentityOrchestratorServer() {
  ...
  return {
    start: async () => {
      ...
      if (redis) startWorker({ redis, pool, inference, s3, logger });
      return addr;
    },
    stop: async () => {
  ...
  app.post("/internal/v1/verifications/:id/run", createRateLimiter({ windowMs: 60_000, limit: 20, keyGenerator: limiterKey }), (req, res, next) => {
    void (async () => {
      ...
      if (isAsync) {
        ...
        if (!opts.redis) throw badRequest("redis_not_configured", "Async execution requires Redis");

        const inserted = await opts.pool.query<{ id: string }>(
        ...
        const queued = await opts.pool.query<{ id: string }>(
      ...
      const running = await opts.pool.query<{ id: string }>(
...
async function purgeOrchestratorRetention(opts: { pool: ReturnType<typeof createPgPool>; days: number; logger: ReturnType<typeof createLogger> }) {
  try {
    ...
    await opts.pool.query(
      "delete from identity_verifications_v2 where updated_at < now() - ($1::int * interval '1 day') and status in ('completed','failed')",
      [days]
    );
  } catch (err) {
