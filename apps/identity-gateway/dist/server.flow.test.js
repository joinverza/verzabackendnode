...
void test("identity-gateway proxies sessions and passes key headers", async () => {
    ...
    await new Promise((resolve) => orchServer.listen(0, "127.0.0.1", resolve));
    const orchAddr = orchServer.address();
    ...
    await gw.stop();
    process.env = prevEnv;
    await new Promise((resolve, reject) => orchServer.close((err) => (err ? reject(err) : resolve())));
});
